...
fn un_champ_absent_du_fichier_reprend_son_defaut() {
    ...
    std::fs::write(
        store::config_path(),
        r#"{"shortcut":"Ctrl+Shift+Y","currency":"EUR"}"#,
    )
    .unwrap();
    let c = store::load_config();
...
fn dossier_en_0700_index_et_configuration_en_0600() {
    ...
    store::save_index(
        index_with(vec![event(trace_core::util::now_ms(), 10)]),
        &Config::default(),
    );
    ...
    assert_eq!(
        mode(store::base_dir()),
        0o700,
        "l'index ne doit pas être lisible par les autres comptes"
    );
    assert_eq!(mode(store::config_path()), 0o600);
    assert_eq!(
        mode(store::index_path()),
        0o600,
        "il porte les noms de projets et les sessions"
    );
}
...
fn la_compaction_replie_a_l_heure_et_conserve_les_totaux() {
    ...
    let folded = r
        .events
        .iter()
        .find(|e| e.compacted.as_deref() == Some("hour"))
        .unwrap();
    assert_eq!(
        folded.tokens.total, 175,
        "aucun token ne doit disparaître au repli"
    );
    assert_eq!(folded.requests, 3);
    assert_eq!(
        folded.session, None,
        "un agrégat horaire recouvre plusieurs sessions : aucune n'est inventée"
    );
}
...
fn un_agregat_deja_replie_n_est_pas_replie_une_seconde_fois() {
    ...
    assert_eq!(
        r.events[0].compacted.as_deref(),
        Some("day"),
        "un repli journalier reste journalier"
    );
...
fn les_evenements_replies_restent_tries_dans_le_temps() {
    ...
    let events = vec![
        event(now, 1),
        event(now - 300 * DAY_MS, 2),
        event(now - 100 * DAY_MS, 3),
    ];
    let r = store::compact(events, 90, now, 0);
...
fn un_elargissement_de_retention_force_une_relecture_complete() {
    ...
    assert!(
        reloaded.collectors.is_empty(),
        "les offsets doivent repartir de zéro"
    );
    assert_eq!(
        reloaded.events.len(),
        1,
        "les événements déjà connus sont conservés"
    );
}
...
fn un_second_processus_n_ecrase_pas_l_index_du_premier() {
    ...
    assert_eq!(
        ...
        written,
        "le fichier du propriétaire est intact"
    );
    assert_eq!(
        result.version, 2,
        "l'appelant reçoit tout de même son index élagué, en mémoire"
    );
}
...
fn relacher_la_propriete_efface_notre_marque_et_pas_celle_d_un_autre() {
    ...
    store::release_ownership();
    assert!(
        store::owner_path().exists(),
        "on n'efface pas la marque d'un autre processus"
    );
}
...
fn un_quota_trop_ancien_est_elague_comme_les_evenements() {
    ...
    idx.quota = vec![
        Quota {
            ...
            resets_at: 0,
            using_overage: false,
            cause: trace_core::collectors::Cause::Unknown,
            used_percent: None,
            window_minutes: None,
            plan: None,
        },
