...
fn croissance_monotone_avec_le_volume() {
    ...
    assert!(
        b > a * 1.9 && b < a * 2.1,
        "doit être quasi linéaire dans le volume"
    );
}
...
fn un_token_genere_coute_bien_plus_qu_un_token_lu_en_cache() {
    ...
    assert!(
        out > cached * 50.0,
        "sortie {out} devrait dominer le cache lu {cached}"
    );
}
...
fn l_eau_suit_l_energie_et_reste_une_fourchette() {
    ...
    let b = est(tk(0, 2000, 0, 0), &m);
    assert!(
        a.water_l.min > 0.0 && a.water_l.min <= a.water_l.mid && a.water_l.mid <= a.water_l.max
    );
    assert!(
        (b.water_l.mid / a.water_l.mid - 2.0).abs() < 0.01,
        "linéaire en volume"
    );
}
...
fn un_fournisseur_inconnu_ouvre_la_fourchette_au_lieu_de_la_flatter() {
    ...
    let unknown = est(
        tk(0, 5000, 0, 0),
        &resolve_model("un-modele-jamais-vu", None),
    );
...
fn la_sensibilite_au_mix_est_ordonnee_et_rapportee_a_la_reference() {
    let m = opus();
    let pairs = vec![Pair {
        tokens: tk(5000, 20_000, 0, 0),
        model: &m,
    }];
    let opts = Options {
        grid_key: Some("us-average".into()),
        ..Options::default()
    };
...
fn la_decomposition_de_l_incertitude_est_triee_du_levier_le_plus_lourd() {
    let m = opus();
    let pairs = vec![Pair {
        tokens: tk(120_000, 8000, 5_000_000, 90_000),
        model: &m,
    }];
    ...
    for w in levers.windows(2) {
        assert!(
            w[0].ratio >= w[1].ratio,
            "les leviers doivent être triés décroissants"
        );
    }
...
fn les_equivalents_sont_lineaires_et_completement_definis() {
    ...
    assert!(
        (car.amount - 10.0).abs() < 1e-9,
        "1200 g / 120 g par km = 10 km"
    );
