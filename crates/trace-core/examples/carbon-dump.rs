...
fn tk(i: i64, o: i64, cr: i64, cw: i64) -> Tokens {
    Tokens { input: i, output: o, cache_read: cr, cache_write: cw, ..Tokens::empty() }
}
...
fn main() {
    ...
    for id in ids.trim().lines() {
        ...
        for v in &vectors {
            ...
            println!(
                "{id}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}",
                f(e.grams_co2e.min), f(e.grams_co2e.mid), f(e.grams_co2e.max),
                f(e.energy_wh.min), f(e.energy_wh.mid), f(e.energy_wh.max),
                f(e.water_l.min), f(e.water_l.mid), f(e.water_l.max),
                f(e.usage_g), f(e.embodied_g), f(e.gpu_count), f(e.grid_intensity),
                e.grid_key,
    ...
    let pairs: Vec<Pair> = models
        .iter()
        .map(|m| Pair { tokens: tk(120_000, 8_000, 5_000_000, 90_000), model: m })
        .collect();

    for r in carbon::grid_sensitivity(&pairs, &opts) {
        println!(
            "GRID\t{}\t{}\t{}\t{}\t{}\t{}",
            r.key, f(r.intensity), f(r.grams_co2e.min), f(r.grams_co2e.mid), f(r.grams_co2e.max),
